...
import KidsSelfy from '../../images/kids_selfy.png';
...
function Senior() {
	...
	useEffect(()=>{
		let applyDataPrimary = applyData.filter(item => item.senior);
		if (applyDataPrimary) {
		applyDataPrimary.forEach(element => {
			element = element.senior;
			setAppoint(element)
		});
		}
	},[applyData])
	...
	return (
		<div className="page__nursery nursery_junior">
			<p className="page__name">KJV / Сад / Старшая группа (5-6 лет)</p>
			...
			<div className="nursery__content">
				<div className='content_description'>
					<h4>Занятия в старшей группе</h4>
					<ul>
				...
				<div className="content_photo"><img src={KidsSelfy} alt="Дети делают селфи-снимок"></img>
				</div>
			</div>
